...
(auxiliary titles, summaries, compaction, memory, vision) that reads
...
def _unwrap_envelope_bytes(raw: bytes) -> bytes:
    ...
    if (
        ...
        # Re-serialize so Content-Length is accurate for the receiver.
        try:
...
def build_clinepass_transport(
    *, async_mode: bool, verify: Any = True, proxy: Any = None
) -> Any:
    """Return an httpx transport that unwraps the ClinePass response envelope.

    Behaves like the plain keepalive transport apart from unwrapping the
    envelope: connection lifecycle is managed at the HTTP pool layer via the
    client's ``keepalive_expiry`` (NOT socket options, which broke streaming
    behind reverse proxies — #54049/#12952), so this transport intentionally
    carries no ``socket_options``. ``verify`` forwards the per-provider SSL
    settings because a mounted transport owns the SSL context for its scheme.
    ``proxy`` is forwarded too: because a mounted transport overrides the
    client-level ``proxy=``, it must carry proxy routing itself so ClinePass
    still honors ``HTTPS_PROXY`` / ``NO_PROXY``.
    """
    import httpx
    ...
    if async_mode:
        ...
        return _ClinePassUnwrapAsyncTransport(verify=verify, proxy=proxy)
    ...
    return _ClinePassUnwrapTransport(verify=verify, proxy=proxy)
