...
class TestDeliverResultStaleTargetFallback:
    ...
    def test_two_dead_targets_redirect_to_home_only_once(self, monkeypatch):
        ...
        # Explicit ``platform:name`` targets go through resolve_send_target,
        # which needs a live directory to map a name to a chat id. Resolve the
        # test's synthetic names to themselves so the test exercises delivery.
        with patch("gateway.config.load_gateway_config", return_value=self._cfg(Platform.DISCORD)), \
             patch("tools.send_message_tool.prepare_send_message_platforms"), \
             patch("tools.send_message_tool.resolve_send_target",
                   side_effect=lambda _platform, rest, **_kw: (rest, None, None)), \
             patch("tools.send_message_tool._send_to_platform", new=send_mock), \
             patch("cron.scheduler.load_config", return_value={"cron": {"wrap_response": False}}):
            result = _deliver_result(job, "reminder")
